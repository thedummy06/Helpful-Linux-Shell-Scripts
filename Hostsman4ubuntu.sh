...
#Paranoidlist
	...
	wget https://github.com/CHEF-KOCH/NSABlocklist/blob/master/Trackers/Canvas%20font%20fingerprinting.txt -O Canvas1
	wget https://github.com/CHEF-KOCH/NSABlocklist/blob/master/Trackers/canvas%20fingerprinting%20pages.txt -O Canvas2
	wget https://github.com/CHEF-KOCH/NSABlocklist/blob/master/Trackers/audio%20fingerporint%20pages.txt -O Audiotracking
	wget https://github.com/CHEF-KOCH/NSABlocklist/blob/master/Trackers/trackers.txt -O Commontracking
	wget https://github.com/CHEF-KOCH/NSABlocklist/blob/master/Trackers/webrtc%20tracking.txt -O Webrtc
	wget https://github.com/CHEF-KOCH/NSABlocklist/blob/master/HOSTS -O NSAlist
	cat Canvas1 >> hosts
	cat Canvas2 >> hosts
	cat Audiotracking >> hosts 
	cat NSAlist >> hosts
	cat Webrtc >> hosts
	cat Commontracking >> hosts
	cat ad_servers.txt >> hosts
	cat Jameshostslist >> hosts
	sed -i 's/127.0.0.1/0.0.0.0/g' hosts
	sort -u hosts > /tmp/hosts.new && mv /tmp/hosts.new hosts && uniq -u hosts >/tmp/hosts.new && mv /tmp/hosts.new hosts
