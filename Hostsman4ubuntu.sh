...
then
	...
	cat unchecky >> hosts
	cat hosts.txt >> hosts
	cat hphosts-partial.txt
	cat cameleonhosts >> hosts
	sed -i 's/0.0.0.0/127.0.0.1/g' hosts
	uniq -u hosts > /tmp/hosts.new && mv /tmp/hosts.new hosts
	rm KADhosts.txt MVPShosts Malwarehosts add.Spam add.Dead add.Risk add.2o7Net Badd-Boyz tyzbit adservers.txt hphosts-partial.txt hosts.txt cameleonhosts spotifyads unchecky
elif [[ $package -eq 6 ]];
then
...
then
#Disconnect lists from Zant95
	wget hosts-file.net/ad_servers.txt 
	wget http://winhelp2002.mvps.org/hosts.txt -O MVPShosts
	wget http://www.malwaredomainlist.com/hostslist/hosts.txt -O malwarelist
	wget https://raw.githubusercontent.com/AdAway/adaway.github.io/master/hosts.txt
	wget https://raw.githubusercontent.com/zant95/hmirror/master/data/disconnect.me-ad/list.txt -O Ads
	wget https://raw.githubusercontent.com/zant95/hmirror/master/data/disconnect.me-malvertising/list.txt -O Malvertising
	wget https://raw.githubusercontent.com/zant95/hmirror/master/data/disconnect.me-malware/list.txt -O Malware
	wget https://raw.githubusercontent.com/zant95/hmirror/master/data/disconnect.me-tracking/list.txt -O Tracking
	cat Ads >> Tracking
	cat Malvertising >> Tracking
	cat Malware >> Tracking	
	sed -i -e 's/^/127.0.0.1 /' Tracking
	cat ad_servers.txt >> Tracking 
	cat malwarelist >> Tracking
	cat hosts.txt >> Tracking
	cat MVPShosts >> Tracking
	cat Jameshostslist >> Tracking
	sed -i 's/0.0.0.0/127.0.0.1/g' Tracking
	uniq -u Tracking > /tmp/Tracking.new && mv /tmp/Tracking.new Tracking
	mv Tracking hosts
else 
