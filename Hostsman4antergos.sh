...
then 
	...
	sed -i 's/0.0.0.0/127.0.0.1/g' hosts
	uniq -u hosts > /tmp/hosts.new && mv /tmp/hosts.new hosts
	rm hphosts-partial.txt cameleonhosts hphosts Jameshostslist readme.txt hosts.txt.asc hosts.zip
elif [[ $package -eq 4 ]];
then 
	wget https://raw.githubusercontent.com/StevenBlack/hosts/master/alternates/fakenews-gambling-porn/hosts
	wget http://www.montanamenagerie.org/hostsfile/hosts.zip
	...
	sed -i 's/0.0.0.0/127.0.0.1/g' hosts
	uniq -u hosts > /tmp/hosts.new && mv /tmp/hosts.new hosts
	rm cameleonhosts hphosts hphosts-partial.txt spamhosts Malwarehosts2 Jameshostslist readme.txt hosts.txt.asc hosts.zip
elif [[ $package -eq 5 ]];
then
